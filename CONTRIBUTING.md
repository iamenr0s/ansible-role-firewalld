...
Thanks for taking the time to contribute to `iamenr0s.ansible_role_firewalld`!
...
Molecule uses Podman locally and Docker in CI. The CI matrix covers Debian, Ubuntu, AlmaLinux, Rocky Linux, and Fedora — changes must not break any supported distro (see the `platforms` list in `meta/main.yml` and the CI matrix in `.github/workflows/molecule.yml`).
